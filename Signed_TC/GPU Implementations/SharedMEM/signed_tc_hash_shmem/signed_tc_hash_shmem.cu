/**
 * @file signed_tc_hash_shmem.cu
 * @brief Driver for Shared-Memory Hash-Based Signed Triangle Counting.
 */
...
#include <gunrock/algorithms/signed_tc_hash_shmem.hxx>
...
void test_signed_tc_hash_shmem(int argc, char **argv) {
  if (argc < 2) {
  ...
  gunrock::signed_tc_hash_shmem::param_t  param;
  gunrock::signed_tc_hash_shmem::result_t result(&total_balanced,
                                                  &total_unbalanced);

  float elapsed =
      gunrock::signed_tc_hash_shmem::run(G, param, result, context);
...
int main(int argc, char **argv) {
  test_signed_tc_hash_shmem(argc, argv);
  return 0;
}
