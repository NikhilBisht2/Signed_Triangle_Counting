/**
 * @file signed_tc_shmem.cu
 * @brief Driver for Shared-Memory Split-CSR Signed Triangle Counting.
 */
...
#include <gunrock/algorithms/signed_tc_shmem.hxx>
...
void test_signed_tc_shmem(int argc, char **argv) {
  if (argc < 2) {
  ...
  gunrock::signed_tc_shmem::param_t  param;
  gunrock::signed_tc_shmem::result_t result(&total_balanced, &total_unbalanced);

  float elapsed = gunrock::signed_tc_shmem::run(G, param, result, context);
...
int main(int argc, char **argv) {
  test_signed_tc_shmem(argc, argv);
  return 0;
}
