...
#include <gunrock/algorithms/signed_tc_merge.hxx>
...
void test_signed_tc_merge(int argc, char** argv) {
  if (argc < 2) {
  ...
  gunrock::signed_tc_merge::param_t  param;
  gunrock::signed_tc_merge::result_t result(&total_balanced, &total_unbalanced);

  float elapsed = gunrock::signed_tc_merge::run(G, param, result, context);
...
int main(int argc, char** argv) {
  test_signed_tc_merge(argc, argv);
  return 0;
}