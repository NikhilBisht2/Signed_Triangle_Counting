/**
 * @file signed_tc_merge.hxx
 * @brief Merge-Based Signed Ranked Triangle Counting on GPU via Gunrock/Essentials.
 *
 * Mirrors the OpenMP pipeline from tc_merge_sign_enu.cpp exactly:
 * 1. Build a degree-ordered forward DAG.
 * 2. Segment-sort each individual vertex's Forward-CSR adjacency lists via parallel insertion.
 * 3. Step through paired neighbor pointers linearly to run intersection validations.
 *
...
struct problem_t : gunrock::problem_t<graph_t> {
  ...
  // Forward-CSR properties built in init() using int64_t offsets [B]
  ...
  void init() override {
    ...
    // ----------------------------------------------------------------
    // Step A: compute safe undirected degrees [A]
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // Step B: stable ranking permutation layout
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // Step C: build standard Forward DAG CSR using int64_t [B]
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // Step D: Inplace sort each vertex's Forward-CSR adjacency sub-list.
    // Necessary to match requirements for linear merge intersections.
    // ----------------------------------------------------------------
    thrust::for_each(
        thrust::device,
        thrust::make_counting_iterator<vertex_t>(0),
        ...
        [f_off_ptr, f_edge_ptr, f_sign_ptr] __device__(vertex_t u) {
          ...
          // In-place dual sorting loop tailored for GPU thread blocks
...
struct enactor_t : gunrock::enactor_t<problem_t> {
  ...
  void loop(gcuda::multi_context_t& context) override {
    ...
    // FIX [D]: Loop over vertices to mirror the CPU implementation exactly
    thrust::for_each(
        thrust::device,
        thrust::make_counting_iterator<vertex_t>(0),
        thrust::make_counting_iterator<vertex_t>(n),
        [f_off, f_e, f_s, rank, d_bal, d_unbal] __device__(vertex_t v) {
          ...
          for (int64_t e = begin_v; e < end_v; ++e) {
            ...
            // Linear Merge Intersection Logic matching intersect_signed_ranked
