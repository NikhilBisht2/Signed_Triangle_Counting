/**
 * @file signed_tc.cu
 * @brief Driver for Sign-Split Triangle Counting on GPU (Gunrock/Essentials).
 */
...
using vertex_t = int;
using edge_t   = int;
using weight_t = float;  // +1.0 = positive edge, -1.0 = negative edge
...
void test_signed_tc(int argc, char** argv) {
  ...
  // Load graph structure and weights natively from the Matrix Market file
  ...
  // Convert the loaded COO layout into the GPU CSR format
  csr.from_coo(coo);

  // Maintain directed=true layout requested by your pipeline logic
  ...
  std::cout << "Edges    : " << G.get_number_of_edges() << "  (both directions; DAG uses half)\n";
