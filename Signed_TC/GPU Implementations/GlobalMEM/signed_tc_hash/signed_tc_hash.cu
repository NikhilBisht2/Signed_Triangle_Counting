/**
 * @file signed_tc_hash.cu
 * @brief Driver for Hash-Based Signed Triangle Counting on GPU (Gunrock/Essentials).
 */
...
#include <gunrock/algorithms/signed_tc_hash.hxx>
...
void test_signed_tc_hash(int argc, char** argv) {
  if (argc < 2) {
  ...
  std::cout << "Edges    : " << G.get_number_of_edges() << " (both directions)\n";

  std::size_t total_balanced   = 0;
  std::size_t total_unbalanced = 0;

  auto context = std::make_shared<gcuda::multi_context_t>(0);
  gunrock::signed_tc_hash::param_t  param;
  gunrock::signed_tc_hash::result_t result(&total_balanced, &total_unbalanced);

  float elapsed = gunrock::signed_tc_hash::run(G, param, result, context);
